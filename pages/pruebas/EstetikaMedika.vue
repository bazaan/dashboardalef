<template>
  <div class="dashboard-container" :data-theme="isDark ? 'dark' : 'light'">
      ...
      <div class="sidebar-header">
        <div class="logo" style="gap: 0.5rem;">
          <div style="width: 35px; height: 35px; border-radius: 50%; overflow: hidden; flex-shrink: 0;">
            <v-img src="@/assets/img/estetika-medika-logo.png" alt="Estetika Medika Logo" style="width: 100%; height: 100%;" />
          </div>
            ...
            <v-menu v-model="showDashboardMenu">
              <template v-slot:activator="{ props }">
                <div v-bind="props" style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                  <span class="logo-text">Estetika Medika</span>
          ...
          <span v-else class="logo-text">Estetika Medika</span>
        </div>
      </div>
    ...
    <div class="main-content">
      ...
      <div v-if="activeView === 'dashboard'" class="view-container">
        ...
        <header class="top-header">
          ...
          <div style="display: flex; gap: 10px; align-items: center;">
            ...
            <N8nPanicButton client-key="estetikaMedika" label="IA Estetika Medika" />
          ...
          <div class="table-section">
            ...
            <v-card flat class="custom-data-table" style="margin-bottom: 2rem;">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(leadsWpp, headersLeadsWpp, 'estetika-leads-wpp')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
            ...
            <v-card flat class="custom-data-table">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" @click="downloadExcel(leadsFbIg, headersLeadsFbIg, 'estetika-leads-fbig')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
              </v-card-title>
          ...
          <div class="table-section">
             <v-card flat class="custom-data-table">
               <v-card-title class="table-search-bar">
                 ...
                 <v-spacer></v-spacer>
                 <v-btn icon size="small" variant="text" color="success" @click="downloadExcel(egresosList, egresosHeaders, 'estetika-egresos')">
                   <v-icon>mdi-file-excel</v-icon>
                   <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                 </v-btn>
               </v-card-title>
      ...
      <!-- ==========  VISTA: FACTURACIÓN (Estetika Medika Version) ========== -->
      ...
      <div v-else-if="activeView === 'contabilidad'" class="view-container">
        ...
        <div v-show="facturacionTab === 'factura_electronica'" style="padding: 0 0 2rem 0;">
          <FacturacionPSE company-id="estetikaMedika" />
        </div>
          ...
          <div class="table-section" :style="{ flex: 1, width: previewUrl ? '50%' : '100%' }">
            <v-card flat class="custom-data-table" style="height: 100%; display: flex; flex-direction: column;">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(medicalHistoryEntries, medicalHistoryHeaders, 'estetika-registros-medicos')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
    ...
    <v-dialog v-model="showPatientFormDialog" max-width="600px" persistent>
      ...
      <div v-if="activeView === 'tickets'" class="view-container">
        <TicketPanel company-id="Estetika Medika" empresa-nombre="Estetika Medika" :current-user="currentUser?.full_name" />
      </div>
  </div>
</template>
...
const fetchPacientesWpp = async () => {
  ...
  try {
    const { data, error } = await client
      .from('PacientesBDwppEstetikaMedika')
      .select('*')
...
const fetchPacientesFbIg = async () => {
  ...
  try {
    const { data, error } = await client
      .from('PacientesBDfbigEstetikaMedika')
      .select('*')
      .order('id', { ascending: false })
...
const fetchLeadsWpp = async () => {
  try {
    ...
    while (true) {
      const { data, error } = await (client
        .from('GeneralBDwppEstetikaMedika')
        .select('*') as any)
        .order('id', { ascending: false })
...
const fetchLeadsFbIg = async () => {
  try {
    const { data, error } = await (client
      .from('GeneralBDfbigEstetikaMedika')
      .select('*') as any)
      .order('id', { ascending: false })
...
const savePatient = async () => {
  ...
  loading.value = true
  try {
    const tableName = selectedPatientType.value === 'wpp' ? 'PacientesBDwppEstetikaMedika' : 'PacientesBDfbigEstetikaMedika'
    ...
    if (selectedPatientType.value === 'wpp') {
      ...
      if (editingPatient.value) {
        const { error } = await (client
          .from('PacientesBDwppEstetikaMedika') as any)
          .update(payload)
      ...
      if (editingPatient.value) {
        const { error } = await (client
          .from('PacientesBDfbigEstetikaMedika') as any)
          .update(payload)
          .eq('id', editingPatient.value.id)
        if (error) throw error
      } else {
    ...
    if (!editingPatient.value) {
      try {
        ...
        const { error: historyError } = await (client
          .from('EstetikaMedika_medical_history') as any)
...
const deletePatient = async (item: any, type: 'wpp' | 'fbig') => {
  ...
  loading.value = true
  try {
    const tableName = type === 'wpp' ? 'PacientesBDwppEstetikaMedika' : 'PacientesBDfbigEstetikaMedika'
...
const facturacionTab = usePersistente('estetikamedika:facturacionTab', 'resumen')
...
const chatItems = [
  { icon: 'mdi-message-reply', label: 'Conversaciones', id: 'conversaciones', url: 'https://chats.alef.company/app/accounts/14/dashboard' }
]
...
const activeTab = usePersistente('estetikamedika:activeTab', 'pacientes_dashboard') // Changed default to pacientes_dashboard from outline
...
async function fetchWorkingHours() {
  try {
    const { data, error } = await (client.from('EstetikaMedika_working_hours') as any).select('*').limit(1).single()
...
async function saveWorkingHours() {
  ...
  try {
    ...
    const { error } = await (client.from('EstetikaMedika_working_hours') as any).upsert(payload, { onConflict: 'id' }).select()
    if (error) throw error
...
async function saveEvent() {
  ...
  try {
    ...
    if (editingEvent.value) {
      // Update
      const { error } = await (client
        .from('EstetikaMedika_calendar_events') as any)
        .update(payload)
...
async function deleteEvent(eventId: string) {
  try {
    const { error } = await client
      .from('EstetikaMedika_calendar_events')
      .delete()
      .eq('id', eventId)
...
async function deleteMultipleMedicalHistory() {
  ...
  if (confirm(`¿Estás seguro de que deseas eliminar ${selectedMedicalHistory.value.length} registros?`)) {
    try {
      ...
      const { error } = await client
        .from('EstetikaMedika_medical_history')
        .delete()
        .in('id', selectedIds)
...
function setupRealtime() {
  client
    .channel('estetika_calendar_events_changes')
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'EstetikaMedika_calendar_events' },
      (payload) => {
...
const fetchEgresos = async () => {
  ...
  const { data, error } = await (client.from('egresos_EstetikaMedika') as any).select('*').order('created_at', { ascending: false })
  if (!error && data) {
...
const openEgresoDialog = (item?: any) => {
  if (item && item.id) {
    ...
    egresoFormData.value = { id: '', tipo_egreso: '', nombre: '', precio: 0, cantidad: 1, company_id: 'estetikaMedika' }
  }
...
const saveEgreso = async () => {
  ...
  if (editingEgreso.value && egresoFormData.value.id) {
    await (client.from('egresos_EstetikaMedika') as any).update(payload).eq('id', egresoFormData.value.id)
  } else {
    await (client.from('egresos_EstetikaMedika') as any).insert(payload)
  }
...
const deleteEgreso = async (id: string) => {
  if (confirm('¿Seguro que deseas eliminar este egreso?')) {
    await (client.from('egresos_EstetikaMedika') as any).delete().eq('id', id)
    fetchEgresos()
  }
}
...
onMounted(() => {
  ...
  if (!canAccessEstetikaMedika(currentUser.value)) {
