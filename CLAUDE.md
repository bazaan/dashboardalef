...
| **Cumplimiento por marca** igual al **Excel de KPIs** (una fila por marca, una columna por tipo de contenido, total, avance %, "el 100 % es todo el mes; en la quincena deberíamos ir en 40–50 %") | Tabla en `PiolaProduccion.vue` + `utils/piolaCumplimiento.ts` (funciones puras con pruebas) **con el formato exacto de la hoja "KPI Y SEGUIMIENTO AGENCIA PIOLA {MES} {AÑO}"**: `CLIENTE | una columna por tipo (lo COMPROMETIDO) | PIEZAS MENSUALES | AVANCE | % DE AVANCE` + fila `TOTALES` (ejemplo de su hoja: 112 piezas, 63 de avance, 56.25 %; Havanna 9+5=14 piezas, 11 de avance = 78.57 %). Cliente en verde, encabezados amarillo y azul, % con **dos decimales** (`-%` si no hay compromiso, como el `#¡DIV/0!` del Excel). El % se pinta según el ritmo contra el **avance esperado a hoy** (día del mes / días del mes): Completo / Al día / En riesgo (hasta 15 puntos por debajo) / Atrasado. Cuánto de cada tipo ya se entregó va en el tooltip de la celda. Sale de la vista `piola_cumplimiento_tipo` (aprobado + entregado): no se llena a mano. El PDF usa el mismo formato (con `print-color-adjust: exact` para que los fondos sí se impriman) |
...
- **Excel de KPIs**: ya llegó (captura de la hoja de septiembre) y la tabla y el PDF de Cumplimiento lo replican. Si Sebastián cambia columnas o el orden de los tipos, se ajusta en `utils/piolaCumplimiento.ts` (`piolaConstruirKpi` / `piolaReporteHtml`).
