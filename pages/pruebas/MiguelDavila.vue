<template>
  <div class="dashboard-container" :data-theme="isDark ? 'dark' : 'light'">
      ...
      <div class="sidebar-header">
        <div class="logo" style="gap: 0.5rem;">
          <div style="width: 35px; height: 35px; border-radius: 50%; overflow: hidden; flex-shrink: 0;">
            <v-img src="@/assets/img/miguel-davila-logo.png" alt="Miguel Davila Logo" style="width: 100%; height: 100%;" />
          </div>
            ...
            <v-menu v-model="showDashboardMenu">
              <template v-slot:activator="{ props }">
                <div v-bind="props" style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                  <span class="logo-text">Miguel Davila</span>
          ...
          <span v-else class="logo-text">Miguel Davila</span>
        </div>
      </div>
    ...
    <div class="main-content">
      ...
      <div v-if="activeView === 'dashboard'" class="view-container">
        ...
        <header class="top-header">
          ...
          <div style="display: flex; gap: 10px; align-items: center;">
            ...
            <N8nPanicButton client-key="davila" label="IA Miguel Davila" />
      ...
      <!-- ==========  VISTA: SETTINGS  ========== -->
      <SettingsView v-else-if="activeView === 'settings'" company-id="Davila" :current-user-role="currentUser?.role" />
      ...
      <div v-else-if="activeView === 'formularios'" class="view-container">
        ...
        <div class="content-area">
          <FormsCompanyPanel company-id="davila" />
        </div>
      </div>
          ...
          <div class="table-section">
            ...
            <v-card flat class="custom-data-table" style="margin-bottom: 2rem;">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(pacientesWpp, headersPacientesWpp, 'davila-pacientes-wpp')">
            ...
            <v-card flat class="custom-data-table">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" @click="downloadExcel(pacientesFbIg, headersPacientesFbIg, 'davila-pacientes-fbig')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
              </v-card-title>
          ...
          <div class="table-section">
            ...
            <v-card flat class="custom-data-table" style="margin-bottom: 2rem;">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(leadsWpp, headersLeadsWpp, 'davila-leads-wpp')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
            ...
            <v-card flat class="custom-data-table">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" @click="downloadExcel(leadsFbIg, headersLeadsFbIg, 'davila-leads-fbig')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
              </v-card-title>
          ...
          <div class="table-section">
             <v-card flat class="custom-data-table">
               <v-card-title class="table-search-bar">
                 ...
                 <v-spacer></v-spacer>
                 <v-btn icon size="small" variant="text" color="success" @click="downloadExcel(egresosList, egresosHeaders, 'davila-egresos')">
                   <v-icon>mdi-file-excel</v-icon>
                   <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                 </v-btn>
               </v-card-title>
      ...
      <!-- ==========  VISTA: FACTURACIÓN (Miguel Davila Version) ========== -->
      ...
      <div v-else-if="activeView === 'contabilidad'" class="view-container">
        ...
        <div v-show="facturacionTab === 'factura_electronica'" style="padding: 0 0 2rem 0;">
          <FacturacionPSE company-id="davila" />
        </div>
          ...
          <div class="table-section">
            <v-card flat class="custom-data-table">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(procedures, procedureHeaders, 'davila-procedimientos')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
          ...
          <div class="table-section" :style="{ flex: 1, width: previewUrl ? '50%' : '100%' }">
            <v-card flat class="custom-data-table" style="height: 100%; display: flex; flex-direction: column;">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(medicalHistoryEntries, medicalHistoryHeaders, 'davila-registros-medicos')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
    ...
    <v-dialog v-model="showPatientFormDialog" max-width="600px" persistent>
      ...
      <div v-if="activeView === 'reporte'" class="view-container">
        <ReporteEmpresaPanel empresa-id="Davila" empresa-nombre="Miguel Davila" :current-user="currentUser?.full_name" />
      </div>
...
import { isSuperAdmin, canAccessDavila, dashboards } from '@/utils/permissions'
...
const savePatient = async () => {
  ...
  loading.value = true
  try {
    const tableName = selectedPatientType.value === 'wpp' ? 'PacientesBDwppDAVILA' : 'PacientesBDfbigDAVILA'
    ...
    if (selectedPatientType.value === 'wpp') {
      ...
      if (editingPatient.value) {
        const { error } = await (client
          .from('PacientesBDwppDAVILA') as any)
          .update(payload)
      ...
      if (editingPatient.value) {
        const { error } = await (client
          .from('PacientesBDfbigDAVILA') as any)
          .update(payload)
          .eq('id', editingPatient.value.id)
        if (error) throw error
      } else {
    ...
    if (!editingPatient.value) {
      try {
        ...
        const { error: historyError } = await (client
          .from('DAVILA_medical_history') as any)
...
const deletePatient = async (item: any, type: 'wpp' | 'fbig') => {
  ...
  loading.value = true
  try {
    const tableName = type === 'wpp' ? 'PacientesBDwppDAVILA' : 'PacientesBDfbigDAVILA'
...
const chatItems = [
  { icon: 'mdi-message-reply', label: 'Conversaciones', id: 'conversaciones', url: 'https://chats.alef.company/app/accounts/3/dashboard' }
]
...
async function fetchWorkingHours() {
  try {
    const { data, error } = await (client.from('DAVILA_working_hours') as any).select('*').limit(1).single()
...
async function saveWorkingHours() {
  ...
  try {
    ...
    const { error } = await (client.from('DAVILA_working_hours') as any).upsert(payload, { onConflict: 'id' }).select()
    if (error) throw error
...
async function saveEvent() {
  ...
  try {
    ...
    if (editingEvent.value) {
      // Update
      const { error } = await (client
        .from('DAVILA_calendar_events') as any)
        .update(payload)
...
function setupRealtime() {
  client
    .channel('davila_calendar_events_changes')
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'DAVILA_calendar_events' },
      (payload) => {
...
const fetchEgresos = async () => {
  ...
  const { data, error } = await (client.from('egresos_DAVILA') as any).select('*').order('created_at', { ascending: false })
  if (!error && data) {
...
const openEgresoDialog = (item?: any) => {
  if (item && item.id) {
    ...
    egresoFormData.value = { id: '', tipo_egreso: '', nombre: '', precio: 0, cantidad: 1, company_id: 'davila' }
  }
...
const saveEgreso = async () => {
  ...
  if (editingEgreso.value && egresoFormData.value.id) {
    await (client.from('egresos_DAVILA') as any).update(payload).eq('id', egresoFormData.value.id)
  } else {
    await (client.from('egresos_DAVILA') as any).insert(payload)
  }
...
const deleteEgreso = async (id: string) => {
  if (confirm('¿Seguro que deseas eliminar este egreso?')) {
    await (client.from('egresos_DAVILA') as any).delete().eq('id', id)
    fetchEgresos()
  }
}
...
onMounted(() => {
  ...
  if (!canAccessDavila(currentUser.value)) {
