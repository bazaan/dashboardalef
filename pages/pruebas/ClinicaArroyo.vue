  ...
  <div class="dashboard-container" :data-theme="isDark ? 'dark' : 'light'">
    ...
    <aside class="sidebar">
      <div class="sidebar-header">
        <div class="logo" style="gap: 0.5rem;">
            ...
            <v-menu v-model="showDashboardMenu">
              <template v-slot:activator="{ props }">
                <div v-bind="props" style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                  <span class="logo-text">Clinica Arroyo</span>
          ...
          <span v-else class="logo-text">Clinica Arroyo</span>
        </div>
      </div>
    ...
    <div class="main-content">
      ...
      <div v-if="activeView === 'dashboard'" class="view-container">
        ...
        <header class="top-header">
          ...
          <div style="display: flex; gap: 10px; align-items: center;">
            ...
            <N8nPanicButton client-key="clinica_arroyo" label="IA Clinica Arroyo" />
          ...
          <div class="table-section">
            ...
            <v-card flat class="custom-data-table" style="margin-bottom: 2rem;">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(pacientesWpp, headersPacientesWpp, 'clinicaarroyo-pacientes-wpp')">
            ...
            <v-card flat class="custom-data-table">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(pacientesFbIg, headersPacientesFbIg, 'clinicaarroyo-pacientes-fbig')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
              </v-card-title>
          ...
          <div class="table-section">
            ...
            <v-card flat class="custom-data-table" style="margin-bottom: 2rem;">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(leadsWpp, headersLeadsWpp, 'clinicaarroyo-leads-wpp')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
            ...
            <v-card flat class="custom-data-table">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(leadsFbIg, headersLeadsFbIg, 'clinicaarroyo-leads-fbig')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
              </v-card-title>
          ...
          <div class="table-section">
             <v-card flat class="custom-data-table">
               <v-card-title class="table-search-bar">
                 ...
                 <v-spacer></v-spacer>
                 <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(egresosList, egresosHeaders, 'clinicaarroyo-egresos')">
                   <v-icon>mdi-file-excel</v-icon>
                   <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                 </v-btn>
               </v-card-title>
          ...
          <div class="table-section">
            <v-card flat class="custom-data-table">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(procedures, procedureHeaders, 'clinicaarroyo-procedimientos')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
          ...
          <div class="table-section" :style="{ flex: 1, width: previewUrl ? '50%' : '100%' }">
            <v-card flat class="custom-data-table" style="height: 100%; display: flex; flex-direction: column;">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(medicalHistoryEntries, medicalHistoryHeaders, 'clinicaarroyo-registros-medicos')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
      ...
      <RemarketingPanel
        ...
        company-id="clinicaarroyo"
        :lead-tablas="{ wpp: 'GeneralBDwppARROYO', fbig: 'GeneralBDfbigARROYO' }"
      />

      <!-- ==========  VISTA: REPORTE DIARIO  ========== -->
      <div v-if="activeView === 'reporte'" class="view-container">
        <ReporteEmpresaPanel empresa-id="Clinica Arroyo" empresa-nombre="Clínica Arroyo" :current-user="currentUser?.full_name" />
      </div>

      <!-- ==========  VISTA: TICKETS  ========== -->
      <div v-if="activeView === 'tickets'" class="view-container">
        <TicketPanel company-id="Clinica Arroyo" empresa-nombre="Clínica Arroyo" :current-user="currentUser?.full_name" />
      </div>
...
const savePatient = async () => {
  ...
  loading.value = true
  try {
    const tableName = selectedPatientType.value === 'wpp' ? 'PacientesBDwppARROYO' : 'PacientesBDfbigARROYO'
    ...
    if (selectedPatientType.value === 'wpp') {
      ...
      if (editingPatient.value) {
        const { error } = await (client
          .from('PacientesBDwppARROYO') as any)
          .update(payload)
      ...
      if (editingPatient.value) {
        const { error } = await (client
          .from('PacientesBDfbigARROYO') as any)
          .update(payload)
          .eq('id', editingPatient.value.id)
        if (error) throw error
      } else {
    ...
    if (!editingPatient.value) {
      try {
        ...
        const { error: historyError } = await (client
          .from('arroyo_medical_history') as any)
...
const deletePatient = async (item: any, type: 'wpp' | 'fbig') => {
  ...
  loading.value = true
  try {
    const tableName = type === 'wpp' ? 'PacientesBDwppARROYO' : 'PacientesBDfbigARROYO'
...
/* ---------------- Estado General ---------------- */
const activeView = useVistaPersistente('clinicaarroyo')
const facturacionTab = usePersistente('clinicaarroyo:facturacionTab', 'resumen')
...
const chatItems = [
  { icon: 'mdi-message-reply', label: 'Conversaciones', id: 'conversaciones', url: 'https://chats.alef.company/app/accounts/5/dashboard' }
]
...
const activeTab = usePersistente('clinicaarroyo:activeTab', 'pacientes_dashboard') // Changed default to pacientes_dashboard from outline
...
async function saveEvent() {
  ...
  try {
    ...
    if (editingEvent.value) {
      // Update
      const { error } = await (client
        .from('arroyo_calendar_events') as any)
        .update(payload)
