/**
 * POST /api/healup/cita-multiple
 *
 * Tool del agente IA: agenda una cita para 2 pacientes a la vez.
 *
 * Reemplaza el subflow n8n "HEALUP | WHATSAPP | tool cita multiple":
 *   1. Crea evento Google Calendar (1 evento con ambos pacientes en descripción)
 *   2. Inserta 1 fila en healup_calendar_events (paciente 1 como principal)
 *   3. Upsert paciente 1 en PacientesBDwppHEALUP (con ID)
 *   4. Insert paciente 2 en PacientesBDwppHEALUP (sin ID)
 *   5. Genera 2 boletas (una por paciente) — solo si healup_boleteo_activo=true
 *   6. Log a agent_tool_logs (tool_name='CitaMultiple')
 *
 * Body esperado:
 * {
 *   api_key:                       string,    — auth
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    console.error('[cita-multiple] Error GCal:', e?.message)
    results.google_calendar = { ok: false, error: e?.message }
  }

  // 6. healup_calendar_events — 1 fila (paciente 1 como principal, ambos en notas)
  try {
    const { data: calData, error: calError } = await (supabase
      ...
      .insert({
        date,
        time,
        subject:        'CITA IA — 2 pacientes',
        procedure_id,
        client_name:    p1.name,
        client_surname: p1.surname,
        client_dni:     dni1,
        client_phone:   phone1,
    ...
    console.error('[cita-multiple] Error calendario:', e?.message)
    results.calendario = { ok: false, error: e?.message }
  }

  // 7. PacientesBDwppHEALUP — Paciente 1 (upsert con ID) + Paciente 2 (insert)
  ...
  try {
    ...
    const op1 = ID
      ? (supabase.from('PacientesBDwppHEALUP') as any).upsert(pac1, { onConflict: 'id' })
      : (supabase.from('PacientesBDwppHEALUP') as any).insert(pac1)
  ...
  // Paciente 2 (siempre insert nuevo, no tiene ID en la BD aún)
  try {
    const pac2 = {
      nombre:             paciente_dos_nombre_completo,
      dni:                dni2,
    ...
    const { data: p2Data, error: e2 } = await (supabase
  ...
  // 8. Boleta — Solo si healup_boleteo_activo=true (1 boleta por paciente)
  try {
    ...
    if (!boleteoActivo) {
      results.boletas = {
        ...
        motivo: 'Boleteo automático desactivado en Healup → Facturación'
      }
    } else {
      ...
      results.boletas = {
        ...
        paciente_uno: {
          serie: boletas[0]?.serie, numero: boletas[0]?.numero,
          enlace_pdf: boletas[0]?.enlace_pdf ?? boletas[0]?.enlace, error: boletas[0]?.error
        },
        paciente_dos: {
          serie: boletas[1]?.serie, numero: boletas[1]?.numero,
          enlace_pdf: boletas[1]?.enlace_pdf ?? boletas[1]?.enlace, error: boletas[1]?.error
        },
      }
    }
  } catch (e: any) {
  ...
  const hayError = [
    results.google_calendar?.ok,
    results.calendario?.ok,
    results.pacientes?.paciente_uno?.ok,
    results.pacientes?.paciente_dos?.ok,
  ].some(v => v === false)
  ...
  console.log(
    `[cita-multiple] Healup | ${p1.name}+${p2.name} | ${date} ${time}`,
