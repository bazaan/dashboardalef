// Healup audit log — escribe en healup_audit_log
// Se llama desde la UI cuando hay cambios sensibles (estado de cita,
// monto, método de pago, edición manual de paciente, soft-delete, etc.)
...
export const useHealupAudit = () => {
  ...
  const log = async (entry: AuditEntry) => {
    try {
      ...
      // Nunca bloqueamos la UI por un fallo de auditoría — solo logueamos.
      console.warn('[HealupAudit] Error registrando log:', err)
    }
  }
