  ...
  <div class="dashboard-container" :data-theme="isDark ? 'dark' : 'light'">
          ...
          <div class="table-section">
            <v-card flat class="custom-data-table">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(compras, headersVentas, 'skip-ventas')">
          ...
          <div class="table-section">
            <v-card flat class="custom-data-table">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(reservas, headersReservas, 'skip-reservas')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
          ...
          <div class="table-section mb-4">
            <v-card flat class="custom-data-table">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(leadsWhatsapp, headersLeadsWhatsapp, 'skip-leads-wpp')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
          ...
          <div class="table-section">
            <v-card flat class="custom-data-table">
              <v-card-title class="table-search-bar">
                ...
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(leadsInstagram, headersLeadsInstagram, 'skip-leads-instagram')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
              </v-card-title>
          ...
          <div class="table-section">
             <v-card flat class="custom-data-table">
               <v-card-title class="table-search-bar">
                 ...
                 <v-spacer></v-spacer>
                 <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(egresosList, egresosHeaders, 'skip-egresos')">
                   <v-icon>mdi-file-excel</v-icon>
                   <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                 </v-btn>
               </v-card-title>
          ...
          <div class="table-section">
            <v-card flat class="custom-data-table">
              <v-card-title class="table-search-bar">
                ...
                <v-spacer></v-spacer>
                <v-btn icon size="small" variant="text" color="success" class="me-1" @click="downloadExcel(procedures, procedureHeaders, 'skip-procedimientos')">
                  <v-icon>mdi-file-excel</v-icon>
                  <v-tooltip activator="parent" location="top">Descargar Excel</v-tooltip>
                </v-btn>
      ...
      <div v-else-if="activeView === 'meta'" class="view-container">
        <header class="top-header">
          <div style="display:flex;flex-direction:column;gap:4px;">
            ...
            <span style="color:var(--text-muted,#888);font-size:0.82rem;">Performance de campañas publicitarias · SKIP Publicidad</span>
          </div>
          <div style="display:flex;gap:10px;align-items:center;">
    ...
    <!-- ==========  SETTINGS DIALOG (REMOVED)  ========== -->
      ...
      <RemarketingPanel
        ...
        company-id="skip"
        :lead-tablas="{ wpp: 'GeneralBDwppSKIP', fbig: 'GeneralBDfbigSKIP' }"
      />

      <!-- ==========  VISTA: REPORTE DIARIO  ========== -->
      <div v-if="activeView === 'reporte'" class="view-container">
        <ReporteEmpresaPanel empresa-id="SKIP" empresa-nombre="SKIP" :current-user="currentUser?.full_name" />
      </div>

      <!-- ==========  VISTA: TICKETS  ========== -->
      <div v-if="activeView === 'tickets'" class="view-container">
        <TicketPanel company-id="SKIP" empresa-nombre="SKIP" :current-user="currentUser?.full_name" />
      </div>
  </div>
</template>
...
/* ---------------- Estado General ---------------- */
const activeView = useVistaPersistente('skip')
const facturacionTab = usePersistente('skip:facturacionTab', 'resumen')
const activeTab = usePersistente('skip:activeTab', 'ventas')
...
const fetchMetaData = async () => {
  ...
  try {
    const [{ data: resumen }, { data: campanas }] = await Promise.all([
      (client.from('SKIP_meta_resumen_mensual') as any)
        .select('*')
        .order('mes', { ascending: true }),
      (client.from('SKIP_meta_campanas') as any)
        .select('*')
        .order('mes', { ascending: true }),
    ])
