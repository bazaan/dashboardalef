/**
 * POST /api/healup/citas-manana-trigger
 *
 * Disparo MANUAL del resumen de citas del día siguiente desde el dashboard
 * ("Probar envío ahora" en el panel "Citas de Mañana" de Healup).
 *
 * Requiere sesión Healup (admin, agente o superadmin con acceso a Healup).
 * Internamente llama a la misma lógica que usa el cron.
 */
...
export default defineEventHandler(async (event) => {
  ...
  // ── 3. Ejecutar el envío ──
  const result = await ejecutarEnvioCitasManana(event, {
    origen: 'manual',
