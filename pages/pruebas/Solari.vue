  ...
  <div class="dashboard-container" :data-theme="isDark ? 'dark' : 'light'">
    ...
    <aside class="sidebar">
      <div class="sidebar-header">
        <div class="logo" style="gap: 0.5rem;">
          ...
          <span v-else class="logo-text">Solari</span>
        </div>
      </div>
    ...
    <div class="main-content">
      ...
      <div v-if="activeView === 'dashboard'" class="view-container">
        ...
        <header class="top-header">
          ...
          <div style="display: flex; gap: 10px; align-items: center;">
            ...
            <N8nPanicButton client-key="solari" label="IA Solari" />
      ...
      <div v-else-if="activeView === 'formularios'" class="view-container">
        ...
        <div class="content-area">
          <FormsCompanyPanel company-id="solari" />
        </div>
      </div>
      ...
      <!-- ==========  VISTA: FACTURACIÓN (Solari Version) ========== -->
      ...
      <div v-else-if="activeView === 'contabilidad'" class="view-container">
        ...
        <div v-show="facturacionTab === 'factura_electronica'" style="padding: 0 0 2rem 0;">
          <FacturacionPSE company-id="solari" />
        </div>
...
const savePatient = async () => {
  ...
  loading.value = true
  try {
    const tableName = selectedPatientType.value === 'wpp' ? 'PacientesBDwppSOLARI' : 'PacientesBDfbigSOLARI'
    ...
    if (selectedPatientType.value === 'wpp') {
      ...
      if (editingPatient.value) {
        const { error } = await (client
          .from('PacientesBDwppSOLARI') as any)
          .update(payload)
      ...
      if (editingPatient.value) {
        const { error } = await (client
          .from('PacientesBDfbigSOLARI') as any)
          .update(payload)
          .eq('id', editingPatient.value.id)
        if (error) throw error
      } else {
    ...
    if (!editingPatient.value) {
      try {
        ...
        const { error: historyError } = await (client
          .from('solari_medical_history') as any)
...
const deletePatient = async (item: any, type: 'wpp' | 'fbig') => {
  ...
  loading.value = true
  try {
    const tableName = type === 'wpp' ? 'PacientesBDwppSOLARI' : 'PacientesBDfbigSOLARI'
...
/* ---------------- Estado General ---------------- */
const activeView = useVistaPersistente('solari')
const facturacionTab = usePersistente('solari:facturacionTab', 'resumen')
...
const activeTab = usePersistente('solari:activeTab', 'pacientes_dashboard') // Changed default to pacientes_dashboard from outline
...
async function fetchWorkingHours() {
  try {
    const { data, error } = await (client.from('solari_working_hours') as any).select('*').limit(1).single()
...
async function saveWorkingHours() {
  ...
  try {
    ...
    const { error } = await (client.from('solari_working_hours') as any).upsert(payload, { onConflict: 'id' }).select()
    if (error) throw error
...
async function saveEvent() {
  ...
  try {
    ...
    if (editingEvent.value) {
      // Update
      const { error } = await (client
        .from('solari_calendar_events') as any)
        .update(payload)
...
function setupRealtime() {
  client
    .channel('solari_calendar_events_changes')
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'solari_calendar_events' },
      (payload) => {
...
const fetchEgresos = async () => {
  ...
  const { data, error } = await (client.from('egresos_solari') as any).select('*').order('created_at', { ascending: false })
  if (!error && data) {
...
const openEgresoDialog = (item?: any) => {
  if (item && item.id) {
    ...
    egresoFormData.value = { id: '', tipo_egreso: '', nombre: '', precio: 0, cantidad: 1, company_id: 'solari' }
  }
...
const saveEgreso = async () => {
  ...
  if (editingEgreso.value && egresoFormData.value.id) {
    await (client.from('egresos_solari') as any).update(payload).eq('id', egresoFormData.value.id)
  } else {
    await (client.from('egresos_solari') as any).insert(payload)
  }
...
const deleteEgreso = async (id: string) => {
  if (confirm('¿Seguro que deseas eliminar este egreso?')) {
    await (client.from('egresos_solari') as any).delete().eq('id', id)
    fetchEgresos()
  }
}
...
onMounted(() => {
  ...
  if (!canAccessSolari(currentUser.value)) {
