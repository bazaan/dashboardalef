<template>
  <div class="dashboard-container" :data-theme="isDark ? 'dark' : 'light'">
    ...
    <aside class="sidebar">
      <div class="sidebar-header">
        <div class="logo" style="gap: 0.5rem;">
          ...
          <span v-else class="logo-text">Brada Perfumes</span>
        </div>
      </div>
    ...
    <div class="main-content">
      ...
      <!-- ==========  VISTA: SETTINGS  ========== -->
      <SettingsView v-else-if="activeView === 'settings'" company-id="Brada" :current-user-role="currentUser?.role" />
      ...
      <div v-else-if="activeView === 'formularios'" class="view-container">
        ...
        <div class="content-area">
          <FormsCompanyPanel company-id="brada" />
        </div>
      </div>
      ...
      <div v-else-if="activeView === 'facturacion'" class="view-container">
        ...
        <div v-show="facturacionTab === 'factura_electronica'" style="padding: 0 0 2rem 0;">
          <FacturacionPSE company-id="bradaperfumes" />
        </div>
        ...
        <div class="content-area">
          ...
          <div class="stats-grid" style="grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));">
            <div class="stat-card">
              <div class="stat-title">Ingresos Brutos (Mes Actual)</div>
            ...
            <div class="stat-card" style="border-left: 4px solid #10b981;">
              ...
              <div class="stat-subtitle">Ingresos Brutos - Egresos</div>
            </div>
      ...
      <div v-else-if="activeView === 'meta'" class="view-container">
        <header class="top-header">
          <div style="display:flex;flex-direction:column;gap:4px;">
            ...
            <span style="color:var(--text-muted,#888);font-size:0.82rem;">Performance de campañas publicitarias · Brada Perfumes</span>
          </div>
          <div style="display:flex;gap:10px;align-items:center;">
      ...
      <div v-if="activeView === 'reporte'" class="view-container">
        <ReporteEmpresaPanel empresa-id="Brada" empresa-nombre="Brada Perfumes" :current-user="currentUser?.full_name" />
      </div>

      <!-- ==========  VISTA: TICKETS  ========== -->
      <div v-if="activeView === 'tickets'" class="view-container">
        <TicketPanel company-id="Brada" empresa-nombre="Brada Perfumes" :current-user="currentUser?.full_name" />
      </div>
...
onMounted(() => {
  // Access Control
  // Access Control
  if (!canAccessBrada(currentUser.value)) {
...
const facturacionTab = usePersistente('bradaperfumes:facturacionTab', 'resumen')
const activeTab = usePersistente('bradaperfumes:activeTab', 'compras')
...
const fetchEgresos = async () => {
  ...
  const { data, error } = await (client.from('egresos_brada') as any).select('*').order('created_at', { ascending: false })
  if (!error && data) {
...
const gananciaNetaTotal = computed(() => {
  // Try to use revenueCurrentMonth if it exists, otherwise 0
  const brutos = typeof revenueCurrentMonth !== 'undefined' ? revenueCurrentMonth.value : 0;
...
const openEgresoDialog = (item?: any) => {
  if (item && item.id) {
    ...
    egresoFormData.value = { id: '', tipo_egreso: '', nombre: '', precio: 0, cantidad: 1, company_id: 'brada' }
  }
...
const saveEgreso = async () => {
  ...
  if (editingEgreso.value && egresoFormData.value.id) {
    await (client.from('egresos_brada') as any).update(payload).eq('id', egresoFormData.value.id)
  } else {
    await (client.from('egresos_brada') as any).insert(payload)
  }
...
const deleteEgreso = async (id: string) => {
  if (confirm('¿Seguro que deseas eliminar este egreso?')) {
    await (client.from('egresos_brada') as any).delete().eq('id', id)
    fetchEgresos()
  }
}
...
const fetchMetaData = async () => {
  ...
  try {
    const [{ data: resumen }, { data: campanas }] = await Promise.all([
      (client.from('BRADA_meta_resumen_mensual') as any)
        .select('*')
        .order('mes', { ascending: true }),
      (client.from('BRADA_meta_campanas') as any)
        .select('*')
        .order('mes', { ascending: true }),
    ])
